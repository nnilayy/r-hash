...
def main():
    ...
    dataset_path = os.path.join(args.root_dir, "dreamer_binary_arousal_dataset.pkl")

    print(
        success(
            f"Ablation Initialized => Ablation: {ABLATION_TAG.upper()} || Dataset: {DATASET_NAME.upper()} || Dimension: {DIMENSION.capitalize()} || Task: {CLASSIFICATION_TYPE.capitalize()} Class Classification"
        )
    )
    ...
    NUM_ELECTRODES = get_num_electrodes(DATASET_NAME)

    # Number of logit classes (SEED=3, DEAP-binary=2, DEAP-multi=9, DREAMER-binary=2, DREAMER-multi=5)
    NUM_CLASSES = get_num_classes(DATASET_NAME, CLASSIFICATION_TYPE)
    ...
    RUN_ID = "0001"
    WANDB_RUN_NAME = f"{ABLATION_TAG_PREFIX}-{PAPER_TASK}-{DATASET_NAME}-{CLASSIFICATION_TYPE}-{DIMENSION}-{TASK_TYPE_SUFFIX}-{ABLATION_TAG}-{RUN_ID}"
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(y_full)))):
        ...
        adasyn = ADASYN(random_state=SEED_VAL)
        ...
        # print(f"\nFold {fold + 1} Training Set Class Balance (After ADASYN):")
